...
#define YFNewfeatureImageCount 4

#import "YFNewfeatureViewController.h"

// 这个地方应该导入的是抽屉式的那个框架,2014
...
- (void)setupScrollView
{
    ...
    for (int i = 0; i< YFNewfeatureImageCount; i++) {
        // 创建UIImageView
        UIImageView *imageView = [[UIImageView alloc] init];
        NSString *name = [NSString stringWithFormat:@"new_feature_%d", i + 1];
#warning 适配 iPhone6和 iPhone6plus 需要再改
        ...
        // 给最后一个imageView添加按钮
        if (i == YFNewfeatureImageCount - 1) {
            [self setupLastImageView:imageView];
        }
    }
    
    // 3.设置其他属性
    scrollView.contentSize = CGSizeMake(YFNewfeatureImageCount * imageW, 0);
    scrollView.pagingEnabled = YES;
    scrollView.showsHorizontalScrollIndicator = NO;
    scrollView.backgroundColor = YFColor(246, 246, 246);
...
- (void)setupPageControl
{
    ...
    // 2.设置圆点的颜色
    pageControl.currentPageIndicatorTintColor = YFColor(253, 98, 42); // 当前页的小圆点颜色
    pageControl.pageIndicatorTintColor = YFColor(189, 189, 189); // 非当前页的小圆点颜色
